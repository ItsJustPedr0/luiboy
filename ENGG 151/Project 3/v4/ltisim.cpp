...
void readSignalFile(string fileName,
                    double*& inputSignal,
                    double*& outputSignal,
                    int& startIndex,
                    int& duration)
{
  ...
  if (getline(dataFile, s1))
  {
    ...
    if (sStream >> s2)
    {
      ...
      if (is_int(s2, &startIndex)
          && (sStream >> firstSignalString))
      {
  ...
  cout << "Signal file '" << fileName
       << "' with starting index " << startIndex
                    ...
                    int& M_plus1, int& N)
{
  ...
  if(M_plus1 == 0 && N == 0)
  {
    cout << "Invalid system file read, "
         << "M+1 and N detected as 0." << endl;
    return;
  }
  ...
  for(int i = 0; i < (int)bVector.size(); i++)
  {
    bCoeff[i] = bVector[i];
  }

  aCoeff = new double[aVector.size()];
  for(int j = 0; j < (int)aVector.size(); j++)
  {
    aCoeff[j] = aVector[j];
  }
...
double computeLTIOutput(double inputSample,
                        double*& bCoeff,
                        double*& aCoeff,
                        int M_plus1, int N,
                        double*& stateX,
                        double*& stateY)
{
  for (int k = M_plus1 - 1; k > 0; k--)
  {
    stateX[k] = stateX[k - 1];
  }
  stateX[0] = inputSample;
