//Filename: gaussFunction.cpp
//Assignment: Project 3: Gauss Elimination
//Name: Peter Estacio
//Grouped with Nino De Mesa and Joshnare Ong
//Section: ENGG 27.01 - M

//Implementation for gaussFunction.h

#include "gaussFunction.h"

fstream readUserInput()
{
  string fileName;
  fstream matrixFile;

  while (true)
  {
    getline(cin, fileName);
    if (fileName.empty())
    {
      matrixFile.open("matrix");
      cout << "\nAugmented Matrix read from 'matrix': " << endl;
      break;
    }
    matrixFile.open(fileName);
    if (!matrixFile.is_open())
    {
      cout << "Error opening " << fileName
           << ". Please try again (defaults to 'matrix'): ";
    }
    else
    {
      cout << "Augmented Matrix read from '" 
        << fileName << "': " << endl;
      break;
    }
  }

  return matrixFile;
}

vector<vector<double>> readMatrixFile(fstream &matrixFile)
{
  string line;
  int rowCount = 0;
  int colCount = 0;

  vector<vector<double>> gaussMatrix;

  while (getline(matrixFile, line))
  {
    istringstream iss(line);
    vector<double> row;
    double val;

    if (rowCount == 0)
    {
      while (iss >> val)
      {
        row.push_back(val);
        cout << val << " | ";
      }
      colCount = row.size();
      cout << endl;

      if (colCount == 0)
      {
        cout << "Invalid matrix: empty row." << endl;
        return {};
      }

      gaussMatrix.push_back(row);
    }
    else
    {
      for (int i = 0; i < colCount; i++)
      {
        if (!(iss >> val))
        {
          cout << "\nInvalid matrix: lacking coefficients." << endl;
          return {};
        }
        row.push_back(val);
        cout << val << " | ";
      }
      cout << endl;
      gaussMatrix.push_back(row);
    }
    rowCount++;
  }

  if (rowCount < colCount - 1)
  {
    cout << "Invalid matrix: lacking equations for number of unknowns."
      << endl;
    return {};
  }

  return gaussMatrix;
}


void gaussElimination(vector<vector<double>> A)
{
  int n = A.size() - 1;
  int cols = A[0].size();
  vector<double> x(n + 1);

  for (int k = 0; k < n; k++)
  {
    int m = k;
    for (int j = k + 1; j < n + 1; j++)
    {
      if (abs(A[m][k]) < abs(A[j][k])) m = j;
    }
    if (A[m][k] == 0)
    {
      cout << "No unique solution exists." << endl;
      return;
    }
    else //exchange row k and m
    {
      vector<double> temp = A[k];
      A[k] = A[m];
      A[m] = temp;
    }
    if (A[n][n] == 0)
    {
      cout << "No unique solution exists." << endl;
      return;
    }
    else
    {
      for(int j = k + 1; j < n + 1; j++)
      {
        double test1 = A[j][k];
        double test2 = A[k][k];
        double m_jk = (A[j][k] / A[k][k]);
        for (int p = k + 1; p < n + 2; p++)
        {
          A[j][p] = A[j][p] - (m_jk * A[k][p]);
        }
      }
    }
  }

  x[n] = (A[n][n+1] / A[n][n]);

  for (int i = (n - 1); i > -1; i--)
  {
    double sum = 0;
    for(int j = i + 1; j < n + 1; j++) sum += A[i][j] * x[j];
    x[i] = (1/A[i][i]) * (A[i][n+1] - sum);
  }
  for (int l = 0; l < x.capacity(); l++) 
  {
    cout << std::fixed << std::setprecision(8) << x[l] << endl;
  }
}
